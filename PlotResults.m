...
for i = 1 : length(t) 
    ...
    if oMeasSensorNormalized(2) >0.5
     ...
     if oMeasSensorNormalized(1)<oMeasSensorNormalized(2) %une verticale est plus proche qu'une horizontale 
        ...
        mRealSensor = oTm \ oRealSensor ; % That's inv(oTm)*oRealMagnet
        ...
        if abs(oMeasSensorNormalized(1)-oMeasSensorNormalized(2))>0.3 %Condition pour que la ligne soit acceptée %ajouté par Charlotte
            dMahaAll(1,j)       = dMaha            ;
...
% Plot Mahalanobis distances. Blue dots are for closest line,
% red dots are for vertical neighbors and yellow ones for horizontal neighbors.
